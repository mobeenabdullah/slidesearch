(function( $ ) {
	...
	var fileUploader = $("#slidesearch-fileuploader").uploadFile({
		...
		extraHTML:function() {
			var html = "<div class='add-tags'>"
					...
					html += "<div class='tagsFieldWrap'><label for='tags'>Add Tags:</label><input type='text' id='slideTagsInput' name='tags' value='tag1,tag2' /></div>";
					html += "<button class='button button-primary' id='showTagsField'><i class='fa fa-plus'></i> Add Tags</button>";
					html += "</div>";
			return html;
		},
		fillTags: function() {
	...
	$('#showTagsField').on('click', function(){
		console.log('test');
	});
